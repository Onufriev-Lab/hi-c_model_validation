...
public:
    ...
    double distance(TadLocation a,double aveX,double aveY,double aveZ); 
    void printMatrix();
    void printMatrix2();
...
double TTMatrix::distance(TadLocation a,double aveX,double aveY,double aveZ)
{
    double dist = sqrt ((pow(a.x - aveX, 2)+ pow(a.y - aveY, 2)+ pow(a.z - aveZ, 2))); //Distance formula
...
{
    int gsize = 225;//TadCount;//change it for X chomosome
...
void TTMatrix::parseVTF(std::ifstream& inFile, std::ifstream& radFile, double tolerance)
{
    ...
    */
    ...
    for (std::string line; getline(inFile,line);) {
        ...
        //for (size_t i = 0; i < tadCount; i++) {//uncomment for all chromosomes
        ...
        for (size_t i = 0; i < 225; i++) {
            //for (size_t i = 993; i < 995; i++) {
            sigma += TTMatrix::distance(tadLocationVector[i], sumX, sumY, sumZ)* distance(tadLocationVector[i], sumX, sumY, sumZ);
            //disCount++;
        
        }
        finalRg =  sqrt((sigma)/225);
