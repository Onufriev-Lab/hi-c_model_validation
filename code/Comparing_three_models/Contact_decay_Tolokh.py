# -*- coding: utf-8 -*-
"""
Created on Sat Jul 15 19:45:03 2023
...
df = pd.read_csv('contact_decay_Tolokh.csv')
...
plt.plot(df['Column1'], df['Ours- Hi-C contact map'], linestyle='solid', color='gold')
...
#xtick_labels = ['100 kb', '1 Mb', '10 Mb', '12 Mb']
xtick_labels = ['10 kb', '1 Mb', '10 Mb', '13 Mb']
xtick_positions = [100000, 1000000, 10000000, 13000000]
...
ytick_labels = [0.8, 2, 3, 4, 6, 8, 10, 12]
ytick_positions = [ 0.8, 2, 3, 4, 6, 8, 10, 12]
...
region_thresholds = [250000, 500000, 1000000, 2000000,10000000]  # Example region thresholds

df = df.sort_values('Column1')
x = df['Column1']
y = df['Ours- Hi-C contact map']
# Create a new figure and plot the existing line
#plt.figure()
#plt.plot(df['Column1'], df['Ours- Hi-C contact map'], label='Existing Line', color='gold')
#plt.yscale('log')
#plt.xscale('log')
# Set the offset value for the line segments (adjust as needed)

# Set the offset value for the line segments (adjust as needed)
offset = (y.max() - y.min()) * 0.003  # Adjust the factor (0.03) as needed
...
plt.xlabel('Genomic Distance'+"\n Ours- Hi-C contact map", fontsize=14)
