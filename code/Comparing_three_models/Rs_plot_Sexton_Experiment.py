...
df = pd.read_csv('contact_decay_Sexton.csv')
...
plt.plot(df['Column1'], df['Sexton- Hi-C contact map'], linestyle='solid', color='r')
...
x = df['Column1']
y = df['Sexton- Hi-C contact map']
# Create a new figure and plot the existing line
#plt.figure()
#plt.plot(df['Column1'], df['Sexton- Hi-C contact map'], label='Existing Line', color = 'red')
#plt.yscale('log')
#plt.xscale('log')
