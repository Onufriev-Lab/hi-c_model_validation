742,	156688742,	140712942,	43905942,	48032142,	64960142,	76069142,	146214542,	89399942,	13964542,	185677942,	414734942,	65806542,	161449742,	41684142,	37240542,	94584142,	127382142,	231595142,	43905942,	82946142,	60410742	,17350142	,92468142	,46550942	,193507142	,192660742	,22640142	,51206142	,134153342,	285024142,	26025742	,114262942	,423727942	,131614142	,263017742	,19995142	,282696542	,22534342	,20947342	,390083542	,180070542	,31315742	,15128342	,90034742	,175309542	,63055742	,15868942	,21687942	,80406942	,65700742	,167162942	,452928742	,118812342	,38404342	,47397342	,110136742	,162719342	,313378542	,48772742	,31315742	,16186342	,36076742	,38192742	,15657342	,230748742	,20629942	,121880542	,44434942	,442560342	,400875142	,260372742	,15868942	,122832742	,134259142	,58506342	,161661342	,29940342	,136163542	,153832142	,110983142	,111300542	,192978142	,20206742	,56496142	,49195942	,175097942	,14176142	,145368142	,28459142	,105375742	,93949342	,122092142	,34807142	,98498742	,68663142	,27083742	,114897742	,31315742	,188640342	,18619742	,342685142	,71519742	,16503742	,29728742	,15339942	,73741542	,86331742	,354111542	,86860742	,49301742	,160074342	,123679142	,21793742	,129921342	,66864542	,60410742	,399182342	,212868542	,18619742	,219216542	,45281342	,19677742	,90881142	,96594342	,213609142	,17455942	,202182742	,125795142	,17455942	,246830342	,284918342	,28035942	,283225542	,53322142	,242386742	,391247342,	49195942};
...
{
    double dist =  (pow(a.x, 2)+pow(a.y, 2)+pow(a.z, 2)); //Distance formula
    //double dist =  (pow(a.x -50, 2)+pow(a.y -50, 2)+pow(a.z -50, 2)); //Distance formula
...
{
    int gsize = 184;//TadCount;//change it for X chromosome
...
void TTMatrix::parseVTF(std::ifstream& inFile, std::ifstream& radFile, double tolerance)
{
    ...
    */
    ...
    for (std::string line; getline(inFile,line);) {
        ...
        //double mss_Li_sum = 0.0;
        double sigma = 0;
        
        //for (const auto& value : mss_Li) {
         //   mss_Li_sum += value;
        //}
        for (size_t i = 986; i < 1169; i++) {
        //for (size_t i = 993; i < 1177; i++) {
            ...
            double squaredDistance = distance(tadLocationVector[i]) * distance(tadLocationVector[i]);
            //for (auto& element : mss_Li) {
            sigma += squaredDistance;
            //}
                    //disCount++;
        ...
        //finalRg =  sqrt((sigma)/mss_Tolokh);
        //finalRg =  sqrt((sigma)/mss_Li_sum);
        finalRg =  sqrt((sigma)/184);
        // Assuming sigma is a double and mss_Li is a vector<double>
