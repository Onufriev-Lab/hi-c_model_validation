...
void TTMatrix::parseVTF(std::ifstream& inFile, std::ifstream& radFile, double tolerance)
{
    ...
    */
    ...
    for (std::string line; getline(inFile,line);) {
        ...
        //double distanceNEW =  TTMatrix::distance(tadLocationVector[993],tadLocationVector[1178]);
        
        double maxDistance = 0.0; // Initialize maxDistance variable to store the maximum distance found
        double max_first_end = 0.0;
        double max_second_end = 0.0;
        for (int i = 993; i < 1178; ++i) {
            for (int j = i + 1; j < 1178; ++j) {
                double distance = TTMatrix::distance(tadLocationVector[i], tadLocationVector[j]);
        ...
        //std::cout << tadLocationVector[993].x << tadLocationVector[993].y << tadLocationVector[993].z << " - " << tadLocationVector[1178].x << tadLocationVector[1178].y << tadLocationVector[1178].z << "      " <<"timeStepCount " << timeStepCount << " distance is " << distanceNEW << "\n";
        std::cout << "timeStepCount " << timeStepCount << " is " << maxDistance << " and the indices are " << max_first_end << " , " << max_second_end << "\n";
