...
SumLastOcc = 0
num_dimensions = 3 #int(input("Input dimensions of square: "))
#Sets dimensions of cubes ^
#print("The TADs have dimensions of",num_dimensions,"by",num_dimensions)
num_steps = 26 #int(input("Input number of steps to be taken: "))
print("========================")
...
Total_Runs = 200000 #((3**8)*6*(4**12))/10
max_found = 5
max_range =  1000
...
for selected_path in All_Paths:   
    ...
    i+=1
    if i % 200 == 0:
        with open("CubesX_2x2.csv", "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerows(Two_Cube_List[100:101])
        print("\nround: "+ str(i) + "/" + str(len(All_Paths)) )
        Two_Cube_List = list()
    for comparison_path in All_Paths:
        Two_Cube_Line = list()
        ...
        if SPE_x == num_dimensions:
            if ( CPS_x == 1 and SPE_y == CPS_y and SPE_z == CPS_z):
                new_path = tuple((x+num_dimensions,y,z) for (x,y,z) in comparison_path[0])
...
print("\nround: "+ str(i) + "/" + str(len(All_Paths)) )   
# with open('CubesX.csv', 'r') as read_obj:
...
#     number_num_occurences.append(dist_count)
